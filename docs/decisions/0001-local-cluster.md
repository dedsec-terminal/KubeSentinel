...
Use k3d as the local Kubernetes distribution for the development cluster.

## Rationale

k3d runs lightweight k3s nodes through Docker Desktop, matching the single-local-cluster scope and available resources. The validated environment uses k3d 5.9.0 with `rancher/k3s:v1.35.5-k3s1`; Helm is required for the pinned Kyverno and Falco deployments.
